...
public class SpecialEventsController {
  ...
  private Ticket[] addSpecialEventTickets(Scanner sc, SpecialEvent event) {
    if (event == null) return null;
    
    Ticket[] tickets = new Ticket[event.getEventType().getNumberOfVisitors()];
    for (int i = 0; i < tickets.length; i++) {
      System.out.print("Add new ticket (Y/N)? ");
      char choice = Character.toUpperCase(sc.next().charAt(0));
      if (choice == 'N') break;
      
      tickets[i] = createTicket(sc, event.getDate());
    }
  ...
  public void findSpecialEvent() {
    for (;;) {
      ...
      switch (choice) {
        ...
        case 2 -> { // Name
          String name = Util.readSpecialEventField(sc, "name");
          SpecialEvent[] events = specialEventsService.getSpecialEvents(name, false);
          if (events == null) System.out.printf("No event(s) found with name '%s'\n", name);
          else printSpecialEvents(events);
        }
        case 3 -> { // Contact number
          String contactNumber = Util.readSpecialEventField(sc, "contact number");
          SpecialEvent[] events = specialEventsService.getSpecialEvents(contactNumber, true);
          if (events == null) System.out.printf("No event(s) found for contact number '%s'\n", contactNumber);
          else printSpecialEvents(events);
        }
        case 4 -> { // Date
          LocalDate date = Util.readSpecialEventDate(sc);
          SpecialEvent[] events = specialEventsService.getSpecialEvents(date);
          if (events == null) System.out.printf("No event(s) found for event date '%s'\n", date);
          else printSpecialEvents(events);
        }
        case 5 -> { // Event type
          EventType eventType = Util.readSpecialEventType(sc);
          SpecialEvent[] events = specialEventsService.getSpecialEvents(eventType);
          if (events == null) System.out.printf("No event(s) found for event type '%s'\n", eventType.name());
          else printSpecialEvents(events);
        }
        case 6 -> {
          // read name
  ...
  public void removeEvents(SpecialEvent[] events) {
